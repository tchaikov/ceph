...
MonConnection& MonClient::_add_conn(unsigned rank)
{
  ...
  MonConnection mc(cct, conn, entity_name,
		  global_id, want_keys, rotating_secrets);
...
int MonClient::_check_auth_rotating()
{
  ...
  if (!active_con || !active_con->get_auth()) {
...
int MonClient::wait_auth_rotating(double timeout)
{
  ...
  assert(active_con && active_con->get_auth());
  auto auth = active_con->get_auth();
...
bool MonConnection::success() const
{
...
int MonConnection::_authenticate(MAuthReply *m)
{
  ...
  if (ret == -EAGAIN) {
    ...
    ldout(cct, 10) << "_send_mon_message to mon." << dendl;
    con->send_message(ma);
  }
  return ret;
}
