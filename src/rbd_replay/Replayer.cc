...
void Replayer::wait_for_actions(const vector<dependency_d> &deps) {
  ...
  BOOST_FOREACH(const dependency_d &dep, deps) {
    ...
    boost::unique_lock<std::mutex> lock(tracker.mutex);
    bool first_time = true;
    ...
    auto sub_release_time(action_completed_time + boost::posix_time::microseconds(dep.time_delta * m_latency_multiplier / 1000));
    if (sub_release_time > release_time) {
      release_time = sub_release_time;
    }
  }
  if (release_time > std::chrono::system_clock::now()) {
    dout(SLEEP_LEVEL) << "Sleeping for " << (release_time - std::chrono::system_clock::now()).total_microseconds() << " microseconds" << dendl;
