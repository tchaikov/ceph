...
TEST_F(onode_tree_test_t, insert_single_leaf)
{
  run_async([this] {
    ...
    seastar::do_with(tm.create_transaction(),
      [&](auto& txn) {
        return btree.insert(oid, onode, *txn).safe_then([&] {
          return tm.submit_transaction(std::move(txn));
        });
      }).unsafe_get();
      // read it back
    auto found = seastar::do_with(tm.create_transaction(),
      [&](auto& txn) {
	return btree.find(oid, *txn).safe_then([&](auto onode) {
          return tm.submit_transaction(std::move(txn)).safe_then([onode] {
	    return Btree::find_ertr::make_ready_future<OnodeRef>(std::move(onode));
          });
        });
    }).unsafe_get0();
    ASSERT_TRUE(found);
