...
struct fltree_onode_manager_test_t
  ...
  void with_onode_write(iterator_t& it, F&& f) {
    with_transaction([this, &it, f=std::move(f)] (auto& t) {
      auto p_kv = *it;
      auto key =  p_kv->key;
      auto onode = INTRV(manager->get_or_create_onode,
          t, key).unsafe_get0();
      std::invoke(f, t, *onode, p_kv->value);
      auto ov = {onode};
      INTRV(manager->write_dirty, t, ov).unsafe_get0();
    });
  }
  ...
  void validate_onode(iterator_t& it) {
    with_transaction([this, &it] (auto& t) {
      auto p_kv = *it;
      auto key = p_kv->key;
      auto onode = INTRV(manager->get_onode,
          t, key).unsafe_get0();
  ...
  void validate_erased(iterator_t& it) {
    with_transaction([this, &it] (auto& t) {
      auto p_kv = *it;
      auto key = p_kv->key;
      auto exist = INTRV(manager->contains_onode,
          t, key).unsafe_get0();
  ...
  void with_onodes_write(
    ...
    with_onodes_process(start, end,
      ...
      auto onodes = INTRV(manager->get_or_create_onodes,
          t, oids).unsafe_get0();
      ...
      INTRV(manager->write_dirty, t, onodes).unsafe_get0();
    });
  }
  ...
  void validate_onodes(
    ...
    with_onodes_process(start, end,
      ...
      for (auto tup : boost::combine(oids, items)) {
        ...
        auto onode = INTRV(manager->get_onode, t, oid).unsafe_get0();
  ...
  void validate_list_onodes(KVPool<onode_item_t>& pool) {
    with_onodes_process(pool.begin(), pool.end(),
      ...
      assert(oids[0] < end);
      while (start != end) {
        auto [list_ret, list_end] = with_trans_intr(
	    t,
	    [&] (auto &t) {
...
TEST_F(fltree_onode_manager_test_t, 1_single)
{
  run_async([this] {
    ...
    with_onode_write(iter, [this](auto& t, auto& onode, auto& item) {
      ...
      INTRV(manager->erase_onode, t, onode_ref).unsafe_get0();
    });
