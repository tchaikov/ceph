...
TEST_F(b_dummy_tree_test_t, 3_random_insert_erase_leaf_node)
{
  run_async([this] {
    ...
    ASSERT_TRUE(INTRV(tree.find, t, key_s).unsafe_get0().is_end());
    ...
    auto f_validate_insert_new = [this, &insert_history] (
      ...
      auto [cursor, success] = INTRV(tree.insert,
      ...
      auto cursor_ = INTRV(tree.find, t, key).unsafe_get0();
    ...
    auto f_validate_erase = [this, &insert_history] (const ghobject_t& key) {
      auto cursor_erase = INTRV(tree.find, t, key).unsafe_get0();
      auto cursor_next = INTR(cursor_erase.get_next, t).unsafe_get0();
      auto cursor_ret = with_trans_intr(
	                  t,
			  [&](auto &t) {
	                    return  tree.erase(t, cursor_erase);
			  }
	                ).unsafe_get0();
      ceph_assert(cursor_erase.is_end());
      ceph_assert(cursor_ret == cursor_next);
      auto cursor_lb = INTRV(tree.lower_bound, t, key).unsafe_get0();
    ...
    // validate lookup
    {
      auto cursor1_s = INTRV(tree.lower_bound, t, key_s).unsafe_get0();
      ASSERT_EQ(cursor1_s.get_ghobj(), key1);
      ASSERT_EQ(cursor1_s.value(), test_value1);
      auto cursor1_e = INTRV(tree.lower_bound, t, key_e).unsafe_get0();
    ...
    // insert the same key1 with a different value
    {
      ...
      auto [cursor1_dup, ret1_dup] = INTRV(tree.insert,
    ...
    for (auto& [k, val] : insert_history) {
      ...
      auto cursor = INTRV(tree.find, t, k).unsafe_get0();
    ...
    {
      auto cursor = INTRV(tree.lower_bound, t, key_s).unsafe_get0();
    ...
    std::random_shuffle(kvs.begin(), kvs.end());
    for (auto& [k, v] : kvs) {
      auto e_size = INTRV(tree.erase, t, k).unsafe_get0();
      ASSERT_EQ(e_size, 1);
    }
    auto cursor = INTR(tree.begin, t).unsafe_get0();
    ASSERT_TRUE(cursor.is_end());
 ...
 public:
  ...
  seastar::future<> split_merge(ghobject_t key, search_position_t pos,
    ...
    return seastar::async([this, key, pos, expected] {
      ...
      auto ctx = pool_clone.get_context();
      with_trans_intr(
	ctx.t,
	[&] (auto &t) {
        return node_to_split->insert_and_split(
	  ctx, key, pool_clone.splitable_nodes);
	}
      ).unsafe_get0();
      {
        std::ostringstream oss;
      ...
      auto ctx2 =  pool_clone.get_context();
      with_trans_intr(
	ctx2.t,
	[&] (auto &t) {
           return node_to_split->merge(
             ctx2, std::move(node_to_split));
	}
      ).unsafe_get0();
  ...
  seastar::future<> fix_index(
	...
	ctx.t,
	[&] (auto &t) {
           return node_to_fix->fix_key(ctx, new_key);
	}
      ).unsafe_get0();
      if (expect_split) {
        std::ostringstream oss;
	...
	auto &pt = pool_clone.t();
      ...
      with_trans_intr(
        ctx2.t,
        [&] (auto &t) {
          return node_to_fix->fix_key(ctx2, old_key);
	}
      ).unsafe_get0();
      auto &pt = pool_clone.t();
      EXPECT_EQ(INTR(pool_clone.p_btree->height, pt).unsafe_get0(), 2);
...
TEST_F(d_seastore_tm_test_t, 6_random_tree_insert_erase)
{
  run_async([this] {
    ...
    // test erase 3/4
    {
      ...
      INTRV(tree->erase, *t, size).unsafe_get();
      submit_transaction(std::move(t));
      segment_cleaner->run_until_halt().get0();
    }
    {
