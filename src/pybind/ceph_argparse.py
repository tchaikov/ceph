...
from typing import cast, Any, Callable, Dict, Generic, List, Optional, Sequence, Tuple, Type, Union
...
class CephArgtype(object):
    ...
    @staticmethod
    def _compound_type_to_argdesc(tp,
                                  attrs: Dict[str, str],
                                  positional: bool) -> str:
...
class CephFloat(CephArgtype):
    ...
    def valid(self, s: str, partial: str = False) -> None:
        try:
            val = float(s)
        except ValueError:
...
class CephPgid(CephArgtype):
    ...
    """
    def valid(self, s: str, partial: bool = False):
        if s.find('.') == -1:
...
class CephFilepath(CephArgtype):
    ...
    Openable file
    """
    def valid(self, s: str, partial: bool = False) -> bool:
...
def validate_command(sigdict: Dict[str, Dict[str, Any]],
    ...
    def grade(cmd: Dict[str, Any]) -> int:
        ...
        sigs = cmd['sig']
        assert isinstance(sigs, argdesc)
