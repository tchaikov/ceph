...
class OrchestratorCli(OrchestratorClientMixin, MgrModule,
    ...
    def _device_light(self,
                      ...
                      devid: str,
                      light_type: Optional[DeviceLightType] = DeviceLightType.ident,
    ...
    def _get_hosts(self, format: Optional[Format] = Format.plain):
        """List hosts"""
        completion = self.get_hosts()
        self._orchestrator_wait([completion])
        raise_if_exception(completion)
        if format != self.Format.plain:
    ...
    def _list_devices(self,
                      ...
                      format: Optional[Format] = Format.plain,
                      refresh: Optional[bool] = False,
                      wide: Optional[bool] = False) -> HandleCommandResult:
        """
    ...
    def _zap_device(self, hostname: str, path: str, force: Optional[bool] = False):
        """
    ...
    def _apply_osd(self,
                   all_available_devices: Optional[bool] = False,
                   format: Optional[Format] = Format.plain,
...
"""
    ...
    def _osd_rm_status(self, format: Optional[Format] = Format.plain) -> HandleCommandResult:
  ...
  ceph orch apply <service_type> [--placement=<placement_string>] [--unmanaged]
    ...
    def _status(self, format: Optional[Format] = Format.plain):
        ...
        if format != Format.plain:
            output = to_format(result, format.name(), many=False, cls=None)
        else:
