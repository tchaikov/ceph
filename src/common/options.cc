...
std::vector<Option> get_global_options() {
  ...
  Option("bluestore_throttle_cost_per_io_hdd", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    ...
    .set_long_description("AVL allocator works in two modes: near-fit and best-fit. By default it tries to fit new block near last allocated block of similar size. This method is very fast. If memory becames scarce switches to best-fit mode, in which it tries to find exact match for requested allocation. This method is considerably slower. When free space is smaller then 'bluestore_avl_alloc_bf_free_pct' mode is best-fit.")
