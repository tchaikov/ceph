...
seastar::future<> SocketConnection::close()
{
  ...
  if (socket) {
    ...
    close_ready = pending_dispatch.close();
  }
...
seastar::future<> SocketConnection::dispatch()
{
...
void SocketConnection::connect(const entity_addr_t& _peer_addr,
...
seastar::future<> SocketConnection::start_connect()
{
...
void SocketConnection::accept(seastar::connected_socket&& sock,
			      const entity_addr_t& peer)
{
  ceph_assert(state == state_t::none);
  ceph_assert(!socket);

  socket.emplace(std::move(sock));
  peer_addr = peer;
