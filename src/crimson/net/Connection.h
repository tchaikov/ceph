...
class Connection : public boost::intrusive_ref_counter<Connection> {
 ...
 public:
  ...
  /// read a message header from a connection that has completed its handshake
  virtual seastar::future<ceph_msg_header> read_header() = 0;

  /// read the remainder of a message after reading its header
