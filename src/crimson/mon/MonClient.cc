                          ...
                          const EntityName& name)
{
  ...
  return conn->send(MessageRef{m, false});
}
...
seastar::future<bool> Connection::do_auth()
{
  ...
  return conn->send(MessageRef{m, false}).then([this] {
    logger().info("waiting");
...
Client::Client(const EntityName& name,
               std::shared_ptr<ceph::net::Messenger> messenger)
  : entity_name{name},
...
Client::get_version_t Client::get_version(const std::string& map)
{
  ...
  return active_con->get_conn()->send(MessageRef{m, false}).then([&req] {
...
Client::run_command(const std::vector<std::string>& cmd,
                    const bufferlist& bl)
{
  auto m = new MMonCommand{monmap.fsid};
