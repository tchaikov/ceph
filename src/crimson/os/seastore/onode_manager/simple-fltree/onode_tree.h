...
public:
                      ...
                      Transaction& txn)
  {
    ...
    return parent.template load_sibling<NodeType>(whoami + 1, txn).safe_then(
      [&parent, whoami, &txn, this](auto&& v) {
      return read_ertr::make_ready_future<update_t>(
        std::visit([&](auto& right) -> update_t {
          const auto [min_grab, max_grab] = me->bytes_to_add(me->used_space());
          if (right.get().used_space() < min_grab) {
            return {};
          }
					   ...
					   DoWith&& do_with)
  {
    ...
    return parent.template load_sibling<NodeType>(whoami - 1, txn).safe_then(
      ...
      return read_ertr::make_ready_future<update_t>(std::visit([&](auto& left) -> update_t {
        ...
        const auto [min_push, max_push] = this->me->bytes_to_remove(new_size);
					    ...
					    DoWith&& do_with)
  {
    ...
    return std::visit([&](auto& right) -> update_t {
      // TODO: downgrade / upgrade elements when moving them across nodes of
      // different layouts
      if constexpr (!std::is_same_v<my_node_t, decltype(right)::my_node_t>) {
        return {};
      }
      const auto [min_push, max_push] = this->me->bytes_to_remove(size);
      if (right.get().free_space() < min_push) {
        return {};
      }
					  ...
					  Transaction& txn)
  {
    ...
    if (count() == 0) {
      ...
      // rebalance
      return grab_or_merge_left(*parent, whoami, txn).safe_then(
        [parent, whoami, &txn, this](auto&& result) {
        if (result) {
          return read_ertr::make_ready_future<update_t>(result);
	} else {
	  // yes, we leave this node underfull without trying harder, because
	  // if we steal elements from its siblings could lead to less optimum
	  // distribution of indexed entries
	  return grab_or_merge_right(*parent, whoami, txn);
        }
      });
    } else {
      return read_ertr::make_ready_future<update_t>();
...
public:
      ...
      return std::visit([&](auto& child) {
        // descend down to my child
          ...
          case result.change_t::removed:
            return this->tm->dec_ref(txn, this->item_at(result.slot)).safe_then(
              [slot=result.slot, &parent, whoami, &txn, this](auto) {
              return base_t::remove_from(slot, parent, whoami, txn);
            });
  ...
  update_key_at(unsigned slot,
    ...
    case size_state_t::underflow:
      ...
      if (parent) {
	return this->grab_or_merge_left(*parent, whoami, txn);
      } else {
        return read_ertr::make_ready_future<update_t>();
      }
    ...
    case size_state_t::overflow: {
      ...
      if (parent) {
        ...
        return this->push_to_left(*parent, whoami, slot, size, txn, update_key).safe_then(
          [parent, whoami, slot, update_key, oid, &txn, this](auto&& update) {
          if (update) {
            return read_ertr::make_ready_future<update_t>(std::move(update));
          } else {
            return read_ertr::make_ready_future<update_t>(std::move(update));
      	    // return this->push_to_right(*parent, whoami, slot, update_key).safe_then(
            //   [parent, whoami, slot, update_key, oid, &txn, this](auto&& update) {
            //   if (update) {
            //     return read_ertr::make_ready_future<update_t>(std::move(update));
            //   } else {
