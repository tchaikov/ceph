...
namespace librbd {
  ...
  int clone(IoCtx& p_ioctx, const char *p_id, const char *p_name,
            ...
            const std::string &non_primary_global_image_id,
            const std::string &primary_mirror_uuid);
  // Standalone clone.  When c_opts has RBD_IMAGE_OPTION_REMOTE_CLUSTER_CONF
  // set (along with the optional REMOTE_KEYRING and REMOTE_CLIENT_NAME)
  // the parent image is looked up in a remote cluster.
