...
def _grant_access(ctx):
    # grant the appropriate access to osd, mgr, mon and mds daemons
    for remote, roles in ctx.cluster.remotes.iteritems():
        for role in roles:
            cluster, _, id_ = teuthology.split_role(role)
            if teuthology.is_type('osd')(role):
                remote.run(
                    args=[
                        'sudo',
                        'ceph',
                        '-i', '/var/lib/ceph/osd/{cluster}-{osd_id}/keyring'.format(
                            clsuter=cluster,
                            osd_id=id_),
                        'auth', 'add',
                        'osd.{osd_id}'.format(osd_id=id_),
                        'osd', 'allow *',
                        'mon', 'allow profile osd',
                        'mgr', 'allow profile osd'
                    ])
            elif teuthology.is_type('mgr')(role):
                remote.run(
                    args=[
                        'sudo',
                        'ceph',
                        '-i', '/var/lib/ceph/mgr/{cluster}-{mgr_id}/keyring'.format(
                            cluster=cluster,
                            mgr_id=id_),
                        'auth', 'add',
                        'mgr.{mgr_id}'.format(mgr_id=id_),
                        'mon', 'allow profile mgr'
                    ])
            elif teuthology.is_type('mds')(role):
                remote.run(
                    args=[
                        'sudo',
                        'ceph',
                        '-i', '/var/lib/ceph/mgr/{cluster}-{mds_id}/keyring'.format(
                            cluster=cluster,
                            mds_id=id_),
                        'auth', 'add',
                        "mds.{mds_id}".format(mds_id=id_),
                        'mon', 'allow profile mds',
                        'osd', 'allow *',
                        'mds', 'allow',
                        'mgr', 'allow profile mds'])
            elif teuthology.is_type('mon')(role):
                # ceph-authtool should have already taken care of monitors
                pass
