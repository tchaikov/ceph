...
#. Let cephadm deploy and configure these services in an opinionated way.
#. Deploy and configure these services yourself.  This is recommended for users
...
#. Deploy prometheus.  A single prometheus instance is sufficient, but
   ...
   Cephadm will handle the prometheus configuration automatically.

#. Deploy grafana::

     ceph orch apply grafana 1

It may take a minute or two for services to be deployed.  Once everything is ready, you should see something like this from ``ceph orch ls``::
...
* Enable the prometheus module in the ceph-mgr daemon so that prometheus can gather Ceph metrics::

     ceph mgr module enable prometheus

  By default, ceph-mgr presents prometheus metrics on port 9283 on each host
  running a ceph-mgr daemon.  Prometheus should be configured to scrape these.
